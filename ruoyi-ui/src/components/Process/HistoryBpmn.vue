<template>
  <div class="bpmnContainers">
    <el-header style="border-bottom: 1px solid rgb(218 218 218);height: auto;">
     ...
     <div class="bpmn-flow-containers">
...
export default {
  ...
  methods: {
    ...
    autoViewport(){
      ...
      const bbox = document.querySelector('.bpmn-flow-containers .viewport').getBBox()
        ...
        flowElements.forEach(n => {
            if (n.$type === 'bpmn:UserTask') {
                ...
                n.outgoing.forEach(nn => {
                    ...
                    if (targetTask) {
                        canvas.addMarker(nn.id, targetTask.completed ? 'highlight' : 'highlight-todo')
                        this.getway(nn.targetRef.$type,nn.targetRef.id,canvas)
                    }
                })

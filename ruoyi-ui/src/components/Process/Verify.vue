  ...
  <div v-if="visible">
    ...
    <el-dialog v-if="visible" title="提交申请" :visible.sync="visible" width="800px" :close-on-click-modal="false"
      ...
      <div v-loading="loading">
        <!--节点开始  -->
        <el-table border :data="processNodeList" v-if="defaultProcess && processNodeList.length > 0" class="processTab"
          max-height="300px">
          ...
          <el-table-column label="操作" align="center" width="100">
            <template slot-scope="scope">
              <el-button size="mini" type="primary" :disabled="!scope.row.disabled"
        ...
        <el-form :rules="rules" ref="formData" :model="formData" status-icon>
          ...
          <el-form-item label="附件" prop="message" v-if="businessStatus.status === 'waiting'" label-width="120px">
            <fileUpload v-model="formData.fileId"/>
          </el-form-item>
          <el-form-item label="提醒方式" prop="sendMessageType" label-width="120px">
            <el-checkbox-group v-model="sendMessageType" size="small">
              <el-checkbox v-for="dict in dict.type.sys_message" :disabled="parseInt(dict.value) === 1"
                :label="parseInt(dict.value)" :key="dict.value" border>
                {{ dict.label }}
              </el-checkbox>
            </el-checkbox-group>
          </el-form-item>
          <el-form-item v-if="nextNodes && nextNodes.length > 0 && !defaultProcess" label="下一步审批人" prop="assigneeMap"
            label-width="120px">
            <div v-for="(item, index) in nextNodes" :key="index">
              ...
              <el-input placeholder="请选择审批人" readonly v-model="nickName[item.nodeId]">
                <el-button @click="choosePeople(item.chooseWay, item.assigneeId, item.nodeId)" slot="append"
                  icon="el-icon-search">选择</el-button>
              </el-input>
            </div>
          </el-form-item>
          <el-form-item label="是否抄送" prop="isCopy" label-width="120px" v-if="setting.isCopy">
            ...
            <el-col :span="12" v-if="formData.isCopy">
              <div class="grid-content bg-purple">
                ...
                <el-input size="small" v-model="formData.assigneeNames" readonly placeholder="请选择人员"
          ...
          <el-form-item align="center">
            ...
            <el-button type="primary" v-if="backNodeList && backNodeList.length > 0 && setting.showBack"
              @click="openBack()" size="small">退回</el-button>
            <el-button type="primary" v-if="isMultiInstance && setting.addMultiInstance" @click="addMultiClick()"
              size="small">加签</el-button>
            <el-button type="primary" v-if="multiList && multiList.length > 0 && setting.deleteMultiInstance"
              @click="deleteMultiClick()" size="small">减签</el-button>
            ...
            <el-button type="primary" v-if="businessStatus.status === 'waiting'" @click="terminationClick()"
              size="small">终止</el-button>
    ...
    <el-dialog :close-on-click-modal="false" title="委托申请" :visible.sync="delegateVisible" width="700px" append-to-body>
      ...
      <sys-dept-user :propUserList="delegateUserList" :multiple=false ref="delegateUserRef"
        @confirmUser="confirmDelegateUser" />
    </el-dialog>
    <!-- 委托申请结束 -->

    <!-- 转办申请开始 -->
    <el-dialog :close-on-click-modal="false" title="转发申请" :visible.sync="transmitVisible" width="700px" append-to-body>
      <el-form ref="transmitData" :model="transferForm" :rules="transmitRules" status-icon>
        <el-form-item label-width="80px" label="审批意见">
          <el-input type="textarea" v-model="transferForm.message" maxlength="300" placeholder="请输入审批意见"
            :autosize="{ minRows: 4 }" show-word-limit></el-input>
        </el-form-item>
        <el-form-item label-width="80px" label="转办人" prop="userName">
          <el-input placeholder="请选择转办人" readonly v-model="transferForm.userName">
      ...
      <sys-dept-user :propUserList="transmitUserList" :multiple=false ref="transmitUserRef"
        @confirmUser="confirmTransmitUser" />
    </el-dialog>
    <!-- 转办申请结束 -->
    ...
    <el-dialog :close-on-click-modal="false" title="加签" :visible.sync="addMultiVisible" width="700px" append-to-body>
      ...
      <multi-user :taskId="taskId" :propUserList="addMultiUserList" ref="addMultiUserRef"
        @confirmUser="confirmAddMultiUser" />
    </el-dialog>
    <!-- 加签结束 -->
...
export default {
  ...
  methods: {
    ...
    transmitSubmit (formName) {
      this.transferForm = {
        transmitUserId: this.transferForm.userId,
        taskId: this.taskId,
        comment: this.transferForm.message
      }
      //消息提醒
      this.transferForm.sendMessage = {
        title: this.messageTitle,
        type: this.sendMessageType,
        messageContent: this.messageContent
      }
      api.transferTask(this.transferForm).then(response => {
        if (response.code === 200) {
          // 刷新数据
          this.$message.success("办理成功");
          // 将表单清空
    ...
    transmitPeople () {
      ...
      this.transmitUserList.push(this.transferForm.userId)
      this.$refs.transmitUserRef.visible = true
    },
    //确认转办人员
    confirmTransmitUser (data) {
      this.transferForm.userId = data[0].userId
      this.transferForm.userName = data[0].nickName
      this.$forceUpdate()

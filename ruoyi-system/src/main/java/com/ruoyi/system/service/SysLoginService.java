...
import com.ruoyi.common.config.SocialProperties;
...
public class SysLoginService {
    ...
    public String renderAuth(String source) {
        if (!socialProperties.getEnabled()) {
            throw new ServiceException("未启用第三方登录！");
        }
        Map<String, SocialProperties.SocialConfigProperties> type = socialProperties.getSource();
        SocialProperties.SocialConfigProperties socialConfigProperties = type.get(source);
        if (socialConfigProperties == null) {
            throw new ServiceException("不支持【" + source + "】登录！");
        }
        AuthRequest authRequest = iSysSocialUserService.getAuthRequest(source,
            socialConfigProperties.getClientId(),
            socialConfigProperties.getClientSecret(),
            socialConfigProperties.getRedirectUri());
    ...
    public Map<String, Object> socialLogin(String source, AuthCallback callback, HttpServletRequest request) {
        Map<String, Object> ajax = new HashMap<>();
        ajax.put("result", AuthResponseStatus.SUCCESS.getCode());
        ...
        AuthRequest authRequest = iSysSocialUserService.getAuthRequest(source,
            ...
            socialConfigProperties.getClientSecret(),
            socialConfigProperties.getRedirectUri());
        AuthResponse<AuthUser> authUserBody = authRequest.login(callback);
        if (authUserBody.getCode() != AuthResponseStatus.SUCCESS.getCode()) {
            ajax.put("result", authUserBody.getMsg());
            return ajax;
        }
        if (StringUtils.isBlank(authorization)) {
            LambdaQueryWrapper<SysSocialUser> wrapper = Wrappers.lambdaQuery();
            wrapper.eq(SysSocialUser::getSource, source);
            wrapper.eq(SysSocialUser::getUuid, authUserBody.getData().getUuid());
            SysSocialUserVo sysSocialUserVo = sysSocialUserMapper.selectVoOne(wrapper);
            if (sysSocialUserVo == null) {
                ajax.put("result", "未绑定【" + source + "】登陆，请先在个人中心绑定！");
                return ajax;
            }
            SysUser sysUser = userMapper.selectUserById(sysSocialUserVo.getSysUserId());
            if (sysUser == null) {
                ajax.put("result", "绑定的用户不存在！");
                return ajax;
            }
            ...
            String tokenValue = authorization.replace("Bearer ", "");
            if (StpUtil.stpLogic.getTokenActiveTimeoutByToken(tokenValue) < -1) {
                ajax.put("result", "登陆过期，请重新登陆！");
                return ajax;
            }
            Object loginIdByToken = StpUtil.getLoginIdByToken(tokenValue);
            String userId = StringUtils.substringAfterLast(loginIdByToken.toString(), "sys_user:");
            AuthUser authUser = authUserBody.getData();
            ajax.putAll(iSysSocialUserService.insertByBo(source, callback, userId, authUser));
            //跳转个人中心
            ajax.put("path", "/user/profile");
        }
        return ajax;
    }
